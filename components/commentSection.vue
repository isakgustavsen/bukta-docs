<template>
  <ul role="list" class="space-y-6 mt-8">
    <li v-for="(activityItem, activityItemIdx) in report_activity" :key="activityItem.id" class="relative flex gap-x-4">
        ...
        <div class="flex-auto rounded-md p-3 ring-1 ring-inset ring-gray-200 dark:ring-slate-700">
          <div class="flex justify-between gap-x-4">
            <div class="py-0.5 text-xs leading-5 text-gray-500">
              <span class="font-medium text-gray-900 dark:text-slate-100">{{ activityItem.name }}</span> kommenterte
            </div>
      ...
      <template v-else>
        <div class="relative flex h-6 w-6 flex-none items-center justify-center bg-white dark:bg-slate-900">
          <UIcon name="i-heroicons-check-circle" v-if="activityItem.type === 'paid'" class="h-6 w-6 text-indigo-600" aria-hidden="true" />
          <div v-else class="h-1.5 w-1.5 rounded-full bg-gray-100 dark:bg-slate-800 ring-1 ring-gray-300 dark:ring-slate-600" />
        </div>
        <p class="flex-auto py-0.5 text-xs leading-5 text-gray-500">
          <span class="font-medium text-gray-900 dark:text-slate-100">{{ activityItem.name }}</span> {{ activityItem.type }} the invoice.
        </p>
        <time :datetime="activityItem.created_at" class="flex-none py-0.5 text-xs leading-5 text-gray-500">{{ format(activityItem.created_at, { date: "medium", time: "medium" }) }}</time>
      </template>
    </li>
  ...
  <div class="mt-6 flex gap-x-3">
    ...
    <UForm @submit="onSubmit" :validate="validate" :state="state"  class="relative flex-auto space-y-2">
      <UFormGroup label="Kommentar" name="comment">
        <UTextarea v-model="state.newComment" placeholder="Skriv inn en kommentar" />
      </UFormGroup>
      <UButton type="submit" label="Comment" class="flex flex-row justify-right" />
    </UForm>
  </div>
...
const validate = (state: any): FormError[] => {
  const errors = []
  if (!state.newComment) errors.push({ path: 'comment', message: 'Påkrevd' })
  return errors
}
